...
namespace sq1code
{
    class ASolver {
        ...
        private static void VisitSolution(AState state, Cube cubeSolution) {
        ...
        private static void SolveSq1Cube(Cube startCube, Predicate<Cube> IsTargetCube, int targetCubeCount, Predicate<Rotation> IsFocusRotation) {
            ...
            Predicate<AState> IsTargetState = (state => state.Depth > 0 && IsTargetCube(state.Cube));
        ...
        private static void OutputState(AState state) {
            Console.WriteLine("cube: {0}", state.Cube);
            Console.WriteLine("depth：{0}", state.Depth);
            do {
                AState fromState = state.BestFrom.Key;
